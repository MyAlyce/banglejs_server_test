...
// ) => {
//     const { userId, name, email, picture, accessToken } = loginData;
...
// const lookupUsers = async (token, userId) => {

//     if (!accessTokens[userId] || accessTokens[userId] !== token) {
//         return false;
//     }
...
const routes = {
    ...
    //         headers: { 'Content-Type': 'application/json' },
    //         handler: (req, res, cfg) => {
    //             lookupUsers(req.headers.authorization, req.headers.userid).then((users) => {
    //                 if(!users) {
    //                     res.writeHead(401, cfg.headers || {});
    //                     res.end('Unauthorized');
    //                 } else {
    //                     res.writeHead(200, { 'Content-Type': 'application/json', ...cfg.headers });
