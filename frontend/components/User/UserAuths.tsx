...
import { DataBase, defaultProfilePic, DataServer, WebRTCclient, webrtcData } from '../../scripts/streamclient';
import Button from 'react-bootstrap/Button';
import { AuthorizationStruct } from 'graphscript-database/src/services/struct/datastructures/types';
...
import { RTCCallProps } from '../../scripts/streamclient';
...
export class UserAuths extends Component<{[key:string]:any}> {
        ...
        return new Promise((res,rej) => {
            DataBase.authorizeUser(
                ...
                DataBase.authorizeUser(
                    ...
                    if(!confirming) await DataBase.addStruct(
                        'authRequest',
                        {
                            requesting:DataBase.currentUser._id, //this will cause this user to receive a notification
                            requestingPictureUrl:DataBase.currentUser.pictureUrl,
                            receiving:userId,
                            receivingName:name,
                            users:[userId,DataBase.currentUser._id],
                            firstName:DataBase.currentUser.firstName,
                            lastName:DataBase.currentUser.lastName,
    ...
    listAuths = async () =>  {
        ...
        let auths = (await DataBase.getAuthorizations())?.filter((a) => {if(a.authorizedId === DataBase.currentUser._id) return true;});
        if(!auths) {
            return;
        } 
        let secondaryAuths = DataBase.getLocalData('authorization', {authorizerId:DataBase.currentUser._id}); //await client.getAuthorizations();
        let userIds = auths.map((v) => {return v.authorizerId});

        let info = await DataBase.getUsers(userIds, true); //get profile pics and stuff

        let onlineUsers = await DataServer.socket.run('usersAreOnline',[userIds]);
        ...
        auths?.forEach((a:AuthorizationStruct) => {
            ...
            let callActive;
            if(userIsOnline) {
                if(WebRTCclient.unansweredCalls) for(const key in WebRTCclient.unansweredCalls) {
            ...
            <ListGroup.Item key={a._id} id={this.unique+a._id}>
                ...
                <div className="float-end">
                    ...
                    <Button variant="outline-success" size="sm" 
                        onClick={async ()=>{ 
                            await DataBase.deleteAuthorization(a._id); 
                            let secondaryAuth = secondaryAuths.find((a2) => {if(a2.authorizedId === a.authorizerId) return true; });
                            if(secondaryAuth) await DataBase.deleteAuthorization(secondaryAuth._id);
        ...
        //my requests
        let authRequests = await DataBase.getData('authRequest', undefined, {requesting: DataBase.currentUser._id});
        ...
        if(authRequests) await Promise.all(authRequests.map(async (req:{
            ...
            if(auths.find((a) => {
                ...
                let cancelRequest = async () => {
                    await DataBase.deleteData([req, ...getAuthsFromRequest(req)]);
                    this.listAuths();
                }
        ...
        let otherAuthRequests = await DataBase.getData('authRequest', undefined, {receiving: DataBase.currentUser._id});
        if(otherAuthRequests) await Promise.all(otherAuthRequests.map( async (req:{
            ...
            if(auths.find((a) => {
                ...
                let reject = () => {
                    DataBase.deleteData([req, ...getAuthsFromRequest(req)]).then(()=>{
                        this.listAuths();
                    });
                }
