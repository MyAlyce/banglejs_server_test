...
import { client, getStreamById, splitCamelCase,  WebRTCclient, 
    webrtcData } from "../../scripts/streamclient";
import { startCall, RTCCallInfo, RTCCallProps, 
    answerCall, disableAudio, disableVideo, 
    enableVideo, enableAudio, sendMessage, 
    callHasMyStreamMedia, rejectCall } from "../../scripts/streamclient";
...
export function AnswerCall(props:{streamId:string, onClick?:(call?)=>void}) {
    return (
        <Button onClick={()=>{
            answerCall(WebRTCclient.unanswered[props.streamId] as any).then(call => {
                //overwrites the default message
                if(props.onClick) props.onClick(call);
            })}}
        >Start Call</Button>
...
export function UnanweredCallInfo(onAnswered?:(call:RTCCallInfo)=>{}) {
    let keys = Object.keys(stateHandler.data.unansweredCalls);
    ...
    for(const key of keys) {
        ...
        let call = stateHandler.data.unansweredCalls[key] as RTCCallProps;
            ...
            <div id={divId} key={divId}>
                ...
                <Button onClick={() => { 
                    answerCall(call as any).then(()=>{
                        if(onAnswered) onAnswered(WebRTCclient.rtc[(call as any)._id] as RTCCallInfo); 
                    });} }>Join Call</Button>
            </div>
        );
    };
...
export function AnswerCallModal (props:{streamId:string}) {
    ...
    let call = stateHandler.data.unansweredCalls[props.streamId] as RTCCallProps;

    const handleClose = () => {
        setShow(false);
        if(stateHandler.data.unansweredCalls[props.streamId]) rejectCall(call._id as string);
    }
        ...
        <Modal centered show={show} onHide={handleClose} backdrop={false}>
            ...
            { call && 
                <Row  className='my-auto'>
                    ...
                    <BeatingSVG customContent={
                        <>{call.pictureUrl ? 
                        <Avatar pictureUrl={call.pictureUrl}/> : 
                        null} | {call.firstName ? `${call.firstName} ${call.lastName}` : null }</>}/>
                        </Col>
                    <Col>
                        <Button 
                            onClick={() => {
...
export class Messaging extends sComponent<{streamId:string, renderMessages?:boolean, renderInput?:boolean}> {
    ...
    render() {
        ...
        const send = (call:RTCCallInfo) => {
            let message = (document.getElementById(this.__unique+'sendmessage') as HTMLInputElement).value;
        ...
        return (<>
            { renderMessages ? <div id={this.__unique + 'messages'}>
                    {this.messages ? this.messages.map(v => v): ""}
                </div> : null 
            }
            { renderInput ? <div>
                    <span  style={{float:'right'}}><input id={this.__unique+'sendmessage'} type='text'></input><Button id={this.__unique+'send'} onClick={()=>{ 
...
export class ViewSelfVideoStream extends sComponent<{streamId:string, style?:string, className?:string}> {
            ...
            <Widget 
                ...
                content={
                    <>
                        ...
                        <Row>
                            <Col>Mic In:</Col>   
                            <Col><select id={this.__unique+'aIn'} onChange={(ev) => {
                        ...
                        <Row>
                            <Col>Audio Out:</Col>
                            <Col><select id={this.__unique+'aOut'} onChange={(ev) => {
                        ...
                        <Row>
                            <Col>Camera In:</Col>
                            <Col><select id={this.__unique+'vIn'} onChange={(ev) => {
...
export class CallSelf extends sComponent {
    ...
    render() {
        ...
        if(stateHandler.data.unansweredCalls) {
            for(const key in stateHandler.data.unansweredCalls) {
                let call = stateHandler.data.unansweredCalls[key] as RTCCallProps;
