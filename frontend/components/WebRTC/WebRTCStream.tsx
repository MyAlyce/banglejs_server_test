...
import { client, WebRTCclient } from "../../scripts/streamclient";
...
import { RTCCallInfo, disableAudio, disableVideo, enableAudio,  enableVideo, getCallerAudioVideo, onrtcdata } from '../../scripts/streamclient';
...
export class RTCAudio extends Component<{
    ...
    setupAudio = (props=this.props) => {
        ...
        if(!props.call && !props.stream) 
            navigator.mediaDevices.getUserMedia({ video: false, audio: stateHandler.data.selectedAudioIn ? {deviceId:stateHandler.data.selectedAudioIn} : true }).then((stream) => { //get your own video if none specified
                this.stream = stream;
                stateHandler.subscribeEvent(
                    'selectedAudioIn', ()=>{
...
export class WebRTCStream extends Component<{[key:string]:any}> {
    ...
    componentDidMount(): void {
        ...
        if(WebRTCclient.rtc[this.props.streamId]) this.setActiveStream(WebRTCclient.rtc[this.props.streamId] as RTCCallInfo);
        let call;
        if(this.state.activeStream) call = WebRTCclient.rtc[this.state.activeStream as string] as RTCCallInfo;
        if(call) this.setupCallUI(call);
    }

    componentWillUnmount(): void {
        let call = WebRTCclient.rtc[this.state.activeStream as string] as RTCCallInfo;
    ...
    setupCallUI(call:RTCCallInfo) {
        ...
        let ontrack = (ev) => {
            ...
            if(ev.track.kind === 'video' && this.state.activeStream === call._id) this.setState({
                videoTrackDiv:createVideoDiv(WebRTCclient.rtc[call._id as any] as any)
            });
            else if(ev.track.kind === 'audio' && this.state.activeStream === call._id) this.setState({
                audioTrackDiv:createAudioDiv(WebRTCclient.rtc[call._id as any] as any, this.audioOutId)
            });
    ...
    render() {
        ...
        if(this.state.activeStream) {
            stream = WebRTCclient.rtc[this.state.activeStream] as RTCCallInfo;
            ...
            stream?.senders?.forEach((s) => {
                ...
                if(s?.track?.kind === 'audio') {
                    ...
                    if((s as any).deviceId && this.audioInId && (s as any).deviceId !== this.audioInId) {
                        disableAudio(WebRTCclient.rtc[this.state.activeStream as any] as any);
                        if(hasVideo && this.audioInId === this.videoInId) {
                            disableVideo(WebRTCclient.rtc[this.state.activeStream as any] as any);
                            enableVideo(WebRTCclient.rtc[this.state.activeStream as any] as any, this.videoInId ? {deviceId:this.videoInId} : undefined, true);
                            videoEnabledInAudio = true;
                        }
                        else enableAudio(WebRTCclient.rtc[this.state.activeStream as any] as any, this.audioInId ? {deviceId:this.audioInId} : undefined);
                    }
                }
                if(s?.track?.kind === 'video') {
                    ...
                    if((s as any).deviceId && this.videoInId && (s as any).deviceId !== this.videoInId && !videoEnabledInAudio) {
                        disableVideo(WebRTCclient.rtc[this.state.activeStream as any] as any);
                        enableVideo(WebRTCclient.rtc[this.state.activeStream as any] as any, this.videoInId ? {deviceId:this.videoInId} : undefined); //todo: deal with case of using e.g. a webcam for both audio and video
                    }
                }
            })
        }
