...
import { DataBase, getStreamById, subscribeToStream, unsubscribeFromStream, WebRTCclient } from '../../scripts/streamclient';
...
import { defaultSpecifiers, genTimestampFromString } from 'graphscript-database';
...
import { processDataForCSV } from '../../scripts/data_util/csv';//'../../../../../graphscript/src/extras/index.storage.services'//
...
export class NoteTaking extends Component<{
    ...
    async listEventHistory() {
        ...
        if(this.streamId) {
            let call = (WebRTCclient.rtc[this.streamId] as RTCCallInfo);
            latest = await DataBase.getData('event', call.caller, searchDict, this.eventLimit, this.eventSkip); //these are gotten in order of the latest data
        } else if (this.userId) {
            latest = await DataBase.getData('event', this.userId, searchDict, this.eventLimit, this.eventSkip); //these are gotten in order of the latest data
        } else {
            latest = await DataBase.getData('event', DataBase.currentUser._id, searchDict, this.eventLimit, this.eventSkip);
        }
                ...
                header={( <>
                    ...
                    <Button onClick={()=>{
                        ...
                        if(this.streamId) {
                            ...
                            name = DataBase.currentUser.firstName + DataBase.currentUser.lastName;
                        }
                        let csvdata = {
                            ...
                            <PopupModal
                                ...
                                <>
                                    { googleAPI.mapsKey ? 
                                        ...
                                        src={`https://www.google.com/maps/embed/v1/place?q=${this.state.selectedCoords}&key=${googleAPI.mapsKey}`}></iframe>
                                ...
                                <tr>
                                    ...
                                    {!this.showInput && <th>
                                        <Button variant={'success'} 
                                            onClick={()=>{ 
                                                stateHandler.setState({[this.streamId ? this.streamId+'notemodal' : 'notemodal']:true}) 
                                            }}
                                        >➕</Button>
                                    </th>}
                                </tr>
