...
import { DataBase, events, getStreamById, WebRTCclient, webrtcData} from '../../scripts/streamclient'
...
import { RTCCallInfo, getCallLocation } from '../../scripts/streamclient';
...
export class NoteForm extends Component<{
    ...
    savedUnits = stateHandler.data.savedUnits as string[];
    ...
    submit = async () => {
        ...
        if(this.streamId) {
            ...
            userId = DataBase.currentUser?._id;
        }
        ...
        let from;
        if(DataBase.currentUser) {
            from = DataBase.currentUser.firstName + DataBase.currentUser.lastName;
        }
    ...
    render() {
        ...
        return (
                ...
                : <>
                    ...
                    <button onClick={()=>{
                        ...
                        if(event && !this.savedEventOptions.includes(event)) {
                            ...
                            stateHandler.setState({savedEventOptions:this.savedEventOptions, writeIn:false});
                            //write saved options to file
                        }
                    }}>Set</button>
                </>
            ...
            <select defaultValue={stateHandler.data.selectedTimeInput} onChange={(ev)=>{
                stateHandler.setState({selectedTimeInput: ev.target.value});
                this.setState({selectedTimeInput:ev.target.value});
            }}>
                <option value="timer">Timer</option>
                <option value="date">DateTime</option>
            </select>
            { stateHandler.data.selectedTimeInput === "date" &&  
                <>
                    <input
                        onChange={(ev)=>{
            ...
            { stateHandler.data.selectedTimeInput === "timer" && 
                <Stopwatch 
                    ...
                    : <>
                        ...
                        <Button onClick={()=>{
                            ...
                            if(units && !this.savedUnits.includes(units)) {
                                ...
                                stateHandler.setState({savedUnits:this.savedUnits, writeInUnits:false});
                                //write saved options to file
                            }
                        }}>Set</Button>
                    </>
