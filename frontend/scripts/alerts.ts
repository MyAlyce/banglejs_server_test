...
import { alerts, client, getStreamById } from "./streamclient";
import { BreathAlert } from "./alertTemplates/breath";
import { FallAlert } from "./alertTemplates/falldetection";
import { Howl } from "howler";
import { recordAlert } from "./recording";
import { webrtcData } from "./streamclient";
import { toISOLocal } from "../scripts/data_util/csv";
...
export function toggleAlertNotifications() {
    stateHandler.setState({alertsEnabled:!stateHandler.data.alertsEnabled});
}
export function toggleHRAlert() {
    stateHandler.setState({useHRAlert:!stateHandler.data.useHRAlert});
}
export function toggleBreathAlert() {
    stateHandler.setState({useBreathAlert:!stateHandler.data.useBreathAlert});
}
export function toggleFallAlert() {
    stateHandler.setState({useFallAlert:!stateHandler.data.useFallAlert});
}
...
export function onAlert(event, streamId?, send?) {
    ...
    if(stateHandler.data.alertsEnabled) {
    ...
    stateHandler.setValue(streamId ? streamId+'alert' : 'alert', event);
}
...
export function setupAlerts(
    ...
    if(!alerts || alerts.includes('hr')) {
        let node = client.add(new HeartRateAlert(
            (ev)=>{ if(stateHandler.data.useHRAlert) onAlert(ev,streamId) },
    ...
    if(!alerts || alerts.includes('breath')) {
        let node = client.add(new BreathAlert(
            (ev)=>{ if(stateHandler.data.useBreathAlert) onAlert(ev,streamId) },
    ...
    if(!alerts || alerts.includes('fall')) {
        let node = client.add(new FallAlert(
            (ev)=>{ if(stateHandler.data.useFallAlert) onAlert(ev,streamId) },
