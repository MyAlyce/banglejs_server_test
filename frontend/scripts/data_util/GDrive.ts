...
export class GFileBrowser extends GDrive {
  ...
  async createFileBrowser(
    ...
    let folderData = await this.checkFolder(folderName) as any;
    if(!folderData) throw new Error("Folder Not Found");
    const fileId = folderData.files[0].id;
    if (!this.directoryId) this.directoryId = fileId;
    await this.updateFileList(fileId, nextPageToken, parentFolder, container);
    this.setupDragAndDrop(fileId, nextPageToken, parentFolder, container);
    this.setupUploadButton(fileId, nextPageToken, parentFolder, container);
    this.setupPaginationButtons(fileId, parentFolder, container);
  }
